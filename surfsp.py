...
# 高速最近傍検索（Fast Library for Approximate Nearest Neighbors)
...
def main():
  ...
  for i in range(ncards):
    ...
    cardColImg[i] = cv2.imread(filename,1)

    # 一番大きい画像のサイズを調べる
    print(np.array(cardColImg[i]).shape)
  ...
  # カメラの取り込み準備
  # capture = cv2.VideoCapture(2) # カム入力の場合    （2とは限らない．0,1,2と試せ）
  ...
  # 2.3の時 (cardKey[i], cardDpt[i]) = surf.detect(cardGrayImg[i],None,False)
    (cardKey[i], cardDpt[i]) = surf.detectAndCompute(cardGrayImg[i],None)
  ...
  while wk != 27:
    ...
    # キャプチャ画像のSURF特徴を抽出する
    # try:
    (camKey, camDpt) = surf.detectAndCompute(camGrayImg[sy1:sy2,sx1:sx2],None,False)
    # except:
    # if len(camDpt) > 0:
    # if camDpt != None:
    print(camDpt.shape)
    if len(camDpt) > FOUNDTH -2 :
    ...
    # 特徴間のマッチング
      ...
      watching = cn
      print("Best is {1:d}, {2:d} pairs with sum of Error {3:f}".format(cn, len(bestpairs),valmin))
